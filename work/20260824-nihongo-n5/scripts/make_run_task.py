"""Generate scripts/run_task.py — drives the full AIOS task pipeline + 7 gates.

This wrapper ensures EVERY AIOS job goes through the real governance pipeline
(spec -> critique x2 -> breakdown -> review -> orchestrate/close) plus the 7
governance gates, and writes a durable log. It is the single entry point that
no AIOS job may bypass.
"""
import os

RUN_TASK = r'''"""run_task.py — run the full AIOS governance pipeline + 7 gates for a TASK-xxx.
