...
## Objective
Triển khai Coding State Machine như một năng lực có contract, evidence và harness riêng (M26 — AIOS 2.0 Coding Edition).

## Scope
- Package: `aios/coding_edition/` (unknown/infra layer, deterministic-first, fail-closed, provenance-bearing).
- Module: `aios/coding_edition/state_machine.py` — class `CodingEditionStateMachine`.
- No LLM, no I/O, no provider/filesystem imports (ARCH-001..004 N/A for unknown layer).

## Deliverables
- Coding State Machine implementation + contract/schema + deterministic tests + evidence + documentation.
