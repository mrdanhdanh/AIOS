...
## Objective
Triển khai **Source / Symbol Index** (M18) như một năng lực có contract, evidence và harness riêng, tích hợp với các task phụ thuộc theo dependency của milestone.

## Scope
**In scope:** `aios/context/symbol_index.py` — `SymbolIndex, SymbolIndexResult, Symbol`.
**Out of scope:** pipeline/retriever mới ngoài phạm vi; vendor lock-in.

## Deliverables
- `aios/context/symbol_index.py` implementation + contract/schema.
- Unit + Contract + Integration + Architecture + Regression tests trong `aios/context/tests/test_context.py`.
- Tích hợp theo dependency: T117 -> T118 -> T120/T121.
...
- Tích hợp dependency theo chuỗi T117 -> T118 -> T120/T121.
